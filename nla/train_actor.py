...
class NLAFSDPActor(FSDPTrainRayActor):
    ...
    def save_model(self, rollout_id, force_sync=False):
        ...
        if self._is_critic_model:
            ...
            # The NCCL-side full gathers of value_head.weight are corrupt
            # (~11k non-finite, even uint16 lanes) via BOTH
            # get_model_state_dict AND DTensor.full_tensor(), while the live
            # local shards, the DCP sharded save, and training loss are all
            # clean/finite (DCP scan of the same iter verified). Not
            # root-caused in torch; value_head is the sole param of the root
            # fully_shard group. Bypass NCCL: gather CPU copies of the local
            # shards over the object (pickle) path and reassemble.
            # COLLECTIVE: all ranks reach this (we're outside the rank-0 gate).
            vh = self.model.value_head.weight
            if isinstance(vh, DTensor):
                rank = dist.get_rank()
                lt = vh.to_local().detach()
                n_bad_local = (~torch.isfinite(lt.float())).sum().item()
                print(
                    f"[NLA][rank {rank}] value_head local shard: dtype={lt.dtype} "
                    f"shape={tuple(lt.shape)} placements={vh.placements} "
                    f"nonfinite={n_bad_local}",
                    flush=True,
                )
                if n_bad_local:
                    raise RuntimeError(
                        f"value_head local shard on rank {rank}: "
                        f"{n_bad_local} non-finite elements"
                    )
                shards = [None] * dist.get_world_size()
                dist.all_gather_object(
                    shards, lt.to("cpu", torch.bfloat16).contiguous()
                )
                # fully_shard shards dim 0; rank order = row order
                vh_full = torch.cat(shards, dim=0)
                if tuple(vh_full.shape) != tuple(vh.shape):
                    raise RuntimeError(
                        f"value_head object-gather shape {tuple(vh_full.shape)} "
                        f"!= expected {tuple(vh.shape)}"
                    )
                n_bad = (~torch.isfinite(vh_full.float())).sum().item()
                if n_bad:
                    raise RuntimeError(
                        f"value_head CPU object gather: {n_bad} non-finite elements"
                    )
                ...
                if isinstance(gathered, torch.Tensor):
                    ...
                    if bad_gather:
                        print(
                            f"[NLA][rank {rank}] full_state_dict gather corrupted "
                            f"value_head.weight ({bad_gather} non-finite) — "
                            f"replaced with CPU object gather",
                            flush=True,
                        )
